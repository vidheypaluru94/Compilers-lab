...
GId	: ID 					{Ginstall($1,vartype,1,NULL);}
	...
	| ID '(' ArgList ')'	{Ginstall($1,vartype,0,Arg_List); Arg_List = NULL; Arg_bind = 0;}
	;
...
Fdef : Type ID '(' ArgList ')' '{' LDefblock Body '}' {	int r,r1;
...
void Argcheck(int type, char *name, struct Argstruct *Arglist){
	...
	if(temp->type == type){
		while(temp1 != NULL) {
			if((strcmp(temp1->name,Arglist->name) == 0) && temp1->type == Arglist->type)
				;
			else {
...
{
	if(root->nodetype == ID_ARRAY)
	{
		if(((root->Gentry)->size > root->value) && root->value >-1)
			;
		else
		{
			printf("error in ID_ARRAY");
			yyerror();
		}
	}
	if(root->nodetype == '+' || root->nodetype == '-' || root->nodetype == '*' || root->nodetype == '/' || root->nodetype == '%')
	{
